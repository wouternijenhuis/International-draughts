# Task 006: Man Movement & Capture Generation
...
Implement legal move generation for men (non-king pieces). This includes non-capture moves (one square diagonally forward) and capture moves (jumping over an adjacent enemy piece forward or backward). Implement the mandatory capture rule: when captures are available, non-capture moves are illegal.
...
### Non-Capture Moves (Men)
- A man can move one square diagonally forward (toward the opponent's side) to an unoccupied square
- Men cannot move backward (except for captures)
...
### Capture Moves (Men)
- A man can capture by jumping over an adjacent enemy piece (diagonally, forward OR backward) to the empty square immediately beyond it
...
1. A man on a non-edge square with two open forward diagonals generates exactly two non-capture moves
2. A man on an edge square generates only valid forward diagonal moves (1 or 0 depending on position)
3. A man blocked by friendly pieces on both forward diagonals generates zero moves
4. A man adjacent to an enemy piece with an empty square beyond generates a capture move
5. A man can capture backward (toward its own side)
...
- **Unit tests:**
  - Man forward moves from various board positions (center, edge, corner-adjacent)
  - Man blocked by friendly and enemy pieces
  - Man captures forward and backward
  ...
  - Edge cases: man at the edge of the board, man surrounded
